Public Class ManPermisos
    ...
    Private Sub ManPermisos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        cAccesoUsuario.LoadCombobox("Accesos", "Acceso", "NombreAcceso", " WHERE Acceso <=" & AccesoUsuario)
        cAccesoUsuarioImportar.LoadCombobox("Accesos", "Acceso", "NombreAcceso", " WHERE Acceso <=" & AccesoUsuario)
    End Sub
    Sub Titulos()
    ...
    Private Sub CargarModulos(ByVal wTabla As String, ByVal wCampo As String, ByVal wCodigo As String)
        ...
        'Swap = SQL("select P.Modulo, M.Texto  from PermisosAcceso P  OIN Modulos M ON P.Modulo = M.Modulo" &
        '           " where Acceso = " & Val(xCodigoImportar.Text))

        'While Not Swap.EOF
        '    xTablaModulos.AddItem("")
        '    xTablaModulos.SetData(xTablaModulos.Rows.Count - 1, T_CHECK, False)
        '    xTablaModulos.SetData(xTablaModulos.Rows.Count - 1, T_MODULO, Swap.Fields("Modulo").Value)
        '    xTablaModulos.SetData(xTablaModulos.Rows.Count - 1, T_TEXTO, Swap.Fields("Texto").Text)
        '    Swap.MoveNext()
        'End While
    ...
    Private Sub bManUsuario_Click(sender As Object, e As EventArgs) Handles bManUsuario.Click
        ...
        If oAccesoA.Checked Then
            cAccesoUsuario.LoadCombobox("Accesos", "Acceso", "NombreAcceso")
        Else
            cAccesoUsuario.LoadCombobox("Usuarios", "Usuario", "NombreUsr")
        End If

        If oAccesoI.Checked Then
            cAccesoUsuarioImportar.LoadCombobox("Accesos", "Acceso", "NombreAcceso")
        Else
    ...
    Private Sub oAccesoA_CheckedChanged(sender As Object, e As EventArgs) Handles oAccesoA.CheckedChanged, oUsuarioA.CheckedChanged
        ...
        ElseIf oUsuarioA.Checked Then
            cAccesoUsuario.LoadCombobox("Usuarios", "Usuario", "NombreUsr", " WHERE Acceso <=" & AccesoUsuario)
    ...
    Private Sub oAccesoI_CheckedChanged(sender As Object, e As EventArgs) Handles oAccesoI.CheckedChanged, oUsuarioI.CheckedChanged
        ...
        ElseIf oUsuarioI.Checked Then
            cAccesoUsuarioImportar.LoadCombobox("Usuarios", "Usuario", "NombreUsr", " WHERE Acceso <=" & AccesoUsuario)
            bAplicarTodo.Visible = False
        End If
        xCodigoImportar.Clear()
    End Sub
